...
from _v1_helpers import evidence_result_for, load_dbs, readiness_result_for
...
def test_ex001_e_coli_at_primary_well_matches_hand_calc(ex001_case):
    ...
    sad = evaluate_site(ex001_case)
    ev = evidence_result_for(ex001_case)
    auth = authorize_screening(
        ex001_case, sad, ev, readiness_result_for(ex001_case, ev)
    )
